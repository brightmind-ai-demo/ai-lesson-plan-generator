...
Format this as a structured lesson plan.`;
    ...
    parseLLMResponse(llmResponse, originalData) {
        ...
        try {
            if (Array.isArray(llmResponse) && llmResponse.length > 0) {
                responseText = llmResponse[0].generated_text || llmResponse[0].text || llmResponse[0] || '';
            } else if (typeof llmResponse === 'string') {
                responseText = llmResponse;
            } else if (llmResponse && llmResponse.generated_text) {
        ...
        return {
            ...
            difficulty: originalData.difficulty || 'intermediate',
    ...
    formatLessonPlan(lesson) {
        ...
        if (lesson.llmGenerated) {
            const serviceName = this.llmServices[this.currentService]?.name || 'AI Service';
            const responsePreview = lesson.llmResponse || 'No response preview available';
            
            basePlan += `
                <div class="lesson-plan-section">
                    <h3>🤖 AI Enhancement</h3>
                    <p><strong>Generated by:</strong> ${serviceName}</p>
                    <p><strong>AI Response Preview:</strong> ${responsePreview}</p>
                </div>
            `;
        }
